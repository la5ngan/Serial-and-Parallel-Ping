...
class ip_check(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            # tampilkan hasilnya
            if n_received:
                self.__successfull_pings = int(n_received[0])

    # fungsi untuk mengetahui status; 0 = tidak ada respon, 1 = hidup tapi ada loss, 2 = hidup
    def status(self):
        # 0 = tidak ada respon
        if self.__successfull_pings == 0:
            return "no response"

        # 1 = ada loss
        elif self.__successfull_pings == 1:
            return "alive, but 50% package loss"
        # 2 = hidup
        elif self.__successfull_pings == 2:
            return "alive"
